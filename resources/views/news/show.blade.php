...
<!-- Article Header -->
<article class="mt-20">
    <header class="relative bg-gradient-to-br from-primary/10 via-sage/10 to-mint/10 py-12 md:py-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Category & Date -->
            <div class="flex items-center gap-4 mb-6">
                @if($news->category)
                <a href="{{ route('news.index', ['category' => $news->category->id]) }}" 
                   class="inline-block px-4 py-2 bg-primary text-white rounded-full text-sm font-medium hover:bg-primary-600 transition-colors duration-200">
                    {{ $news->category->name }}
                </a>
                @endif
                <time class="text-gray-600 text-sm font-medium" datetime="{{ $news->published_at->toISOString() }}">
            ...
            <div class="flex flex-wrap items-center gap-6 text-gray-600">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium">{{ $news->user->name }}</span>
                </div>
                @endif
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm">{{ number_format($news->views) }} kali dibaca</span>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm">{{ $news->created_at ? $news->created_at->diffForHumans() : 'Baru saja' }}</span>
                </div>
            </div>
        </div>
    </header>
            ...
            <div class="lg:col-span-2">
                ...
                <figure class="mb-8 rounded-2xl overflow-hidden shadow-lg">
                    <img src="{{ Storage::url($news->featured_image) }}" 
                         ...
                         class="w-full h-auto">
                    @if($news->excerpt)
                    <figcaption class="bg-gray-50 px-6 py-4 text-gray-600 italic">
                        {{ $news->excerpt }}
                    </figcaption>
                    @endif
                </figure>
                ...
                <div class="mt-12 pt-8 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <h4 class="text-lg font-semibold text-gray-900">Bagikan Berita Ini:</h4>
                        <div class="flex items-center gap-3">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('news.show', $news->slug)) }}&text={{ urlencode($news->title) }}" 
                               target="_blank"
                               class="w-10 h-10 bg-sky-500 text-white rounded-full flex items-center justify-center hover:bg-sky-600 transition-colors duration-200">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <aside class="lg:col-span-1">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6 mb-8 sticky top-24">
                    ...
                    <div class="space-y-6">
                        ...
                        <article class="group">
                            <a href="{{ route('news.show', $item->slug) }}" class="block">
                                <h4 class="font-semibold text-gray-900 mb-2 line-clamp-2 group-hover:text-primary transition-colors duration-200">
                                    {{ $item->title }}
                                </h4>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <a href="{{ route('news.index') }}" 
                           class="block text-center py-2 text-primary font-semibold hover:text-primary-600 transition-colors duration-200">
                            Lihat Semua Berita →
                        </a>
                    </div>
                </div>
...
<section class="py-16 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <article class="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group hover:-translate-y-1">
                <a href="{{ route('news.show', $item->slug) }}">
                    ...
                    @else
                    <div class="h-40 bg-gradient-to-br from-sage to-mint flex items-center justify-center">
                    ...
                    <div class="p-4">
                        ...
                        <h3 class="font-bold text-gray-900 mt-2 line-clamp-2 group-hover:text-primary transition-colors duration-200">
                            {{ $item->title }}
                        </h3>
                        @if($item->excerpt)
