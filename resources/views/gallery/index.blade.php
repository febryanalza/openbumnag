...
<div x-data="galleryModal()">
    ...
    <section class="py-8 sm:py-12 lg:py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-0.5 sm:gap-1 md:gap-2">
                ...
                <div class="relative aspect-square bg-gray-200 overflow-hidden group cursor-pointer"
                    ...
                    @else
                        ...
                        <div class="absolute top-1.5 right-1.5 sm:top-2 sm:right-2">
                            <svg class="w-4 h-4 sm:w-6 sm:h-6 text-white drop-shadow-lg" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="text-white text-center">
                            <div class="flex items-center justify-center space-x-4">
                                ...
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 sm:w-6 sm:h-6 mr-1" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/>
                                    </svg>
                                    <span class="font-semibold text-sm sm:text-base">{{ number_format($gallery->views) }}</span>
                                </div>
                                @endif
                            </div>
                        </div>
            ...
            <div class="text-center py-12 sm:py-16 lg:py-20">
                ...
                <h3 class="text-xl sm:text-2xl font-bold text-gray-700 mb-2">Belum Ada Galeri</h3>
                <p class="text-sm sm:text-base text-gray-500 px-4">Galeri akan ditampilkan di sini.</p>
            </div>
            @endif
        </div>
    </section>
    ...
    <div x-show="isOpen" 
        ...
        <div class="relative w-full h-full max-w-7xl mx-auto flex items-center justify-center p-2 sm:p-4">
            <!-- Close Button -->
            <button @click="closeModal()" 
                    class="absolute top-2 right-2 sm:top-4 sm:right-4 z-10 text-white hover:text-gray-300 transition-colors touch-target">
            ...
            <button @click="previousImage()" 
                    ...
                    class="absolute left-1 sm:left-4 text-white hover:text-gray-300 transition-colors z-10 touch-target">
            ...
            <button @click="nextImage()" 
                    ...
                    class="absolute right-1 sm:right-4 text-white hover:text-gray-300 transition-colors z-10 touch-target">
                <svg class="w-8 h-8 sm:w-10 sm:h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Modal Content -->
            <div class="bg-white rounded-none sm:rounded-lg overflow-hidden flex flex-col md:flex-row max-h-[95vh] sm:max-h-[90vh] w-full md:w-auto shadow-2xl">
                <!-- Image/Video Section -->
                <div class="flex-shrink-0 bg-black flex items-center justify-center md:max-w-[60vw]">
                    <template x-if="currentGallery && currentGallery.file_type === 'image'">
                        <img :src="currentGallery.file_url" 
                             ...
                             class="max-h-[40vh] sm:max-h-[50vh] md:max-h-[90vh] w-auto object-contain">
                    </template>
                    <template x-if="currentGallery && currentGallery.file_type === 'video'">
                        <video controls class="max-h-[40vh] sm:max-h-[50vh] md:max-h-[90vh] w-auto">
                ...
                <div class="w-full md:w-80 lg:w-96 flex flex-col bg-white max-h-[45vh] sm:max-h-[40vh] md:max-h-[90vh]">
                    <!-- Header -->
                    <div class="p-3 sm:p-4 border-b border-gray-200">
                        <div class="flex items-center">
                            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-primary to-sage rounded-full flex items-center justify-center flex-shrink-0">
                                <svg class="w-4 h-4 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                </svg>
                            </div>
                            <div class="ml-2 sm:ml-3 min-w-0">
                                <p class="font-semibold text-gray-900 text-sm sm:text-base truncate" x-text="currentGallery?.photographer || 'BUMNag Lubas Mandiri'"></p>
                                <p class="text-xs text-gray-500 truncate" x-text="currentGallery?.location"></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 overflow-y-auto p-3 sm:p-4">
                        <div class="mb-3 sm:mb-4">
                            <h3 class="text-base sm:text-lg font-bold text-gray-900 mb-1 sm:mb-2" x-text="currentGallery?.title"></h3>
                            <p class="text-sm sm:text-base text-gray-700 whitespace-pre-line" x-text="currentGallery?.description"></p>
                        </div>
                        ...
                        <div class="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-gray-600">
                            ...
                            <div class="flex items-center" x-show="currentGallery?.album">
                                ...
                                <span class="truncate">Album: <span x-text="currentGallery?.album"></span></span>
                            </div>
                            <div class="flex items-center">
                                <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
