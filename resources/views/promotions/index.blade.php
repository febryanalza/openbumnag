            ...
            <div class="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 mb-8 sm:mb-12">
                ...
                <article class="bg-white rounded-xl sm:rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group hover:-translate-y-2 border-2 border-amber-200">
                    ...
                    <div class="p-4 sm:p-5">
                        ...
                        <!-- Rating -->
                        @if($promotion->reviews_count > 0)
                        <div class="flex items-center gap-1.5 mb-3 sm:mb-4">
                            <div class="flex items-center">
                                @php $avgRating = round($promotion->average_rating ?? 0, 1); @endphp
                                @for($i = 1; $i <= 5; $i++)
                                    ...
                                    @elseif($i - 0.5 <= $avgRating)
                                        <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                                            <defs><linearGradient id="half-promo-{{ $promotion->id }}"><stop offset="50%" stop-color="currentColor"/><stop offset="50%" stop-color="#D1D5DB"/></linearGradient></defs>
                                            <path fill="url(#half-promo-{{ $promotion->id }})" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                        </svg>
                                    @else
                            ...
                            <span class="text-xs text-gray-400">({{ $promotion->reviews_count }})</span>
                        </div>
                        @endif
